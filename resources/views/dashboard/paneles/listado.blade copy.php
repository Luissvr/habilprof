...
{{-- DEBUG: probar si se imprime algo simple --}}
<h2 style="color: green">DEBUG: listado.blade.php se está renderizando</h2>

@if(isset($habilitaciones))
    <p style="color: blue">
        Total habilitaciones: {{ $habilitaciones->count() }}
    </p>

    @foreach($habilitaciones as $hab)
        <p style="color: purple">
            {{ $hab->id_habilitacion ?? 'sin id' }} -
            {{ $hab->rut_alumno ?? 'sin rut' }}
        </p>
        @break  {{-- mostramos solo 1 para no llenar la pantalla --}}
    @endforeach
@else
    <p style="color: red">NO existe la variable $habilitaciones</p>
@endif
