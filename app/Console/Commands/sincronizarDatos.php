...
class SincronizarDatos extends Command
{
    ...
    public function handle()
    {
        ...
        // Si la BD 'pgsql_ucsc' está caída o algo falla, el 'catch' lo atrapará.
        try {
            ...
            // El Scheduler (Kernel) esperará 60s para el próximo ciclo.
            $this->error('¡LA TAREA PROGRAMADA FALLÓ!');
            $this->error($e->getMessage()); // Muestra el error en tu consola
            Log::error('¡ERROR EN TAREA PROGRAMADA! ' . $e->getMessage()); // Lo guarda en laravel.log
        }
    }

    /**
     * Sincroniza Alumnos (R1.1, R1.2, R1.7.1)
     */
    private function sincronizarAlumnos()
    {
        $this->info('Iniciando sincronización de alumnos...');
        $alumnosExternos = DB::connection('pgsql_ucsc')->table('public.nomina_alumnos_externos')->get();
        ...
        foreach ($alumnosExternos as $externo) {
            ...
            // 'updateOrCreate' es la magia de Laravel:
    ...
    private function sincronizarProfesores()
    {
        ...
        $profesoresExternos = DB::connection('pgsql_ucsc')->table('public.nomina_profesores_externos')->get();
        ...
        foreach ($profesoresExternos as $externo) {
            ...
            Profesor::updateOrCreate(
                ...
                [ // Datos a insertar/actualizar
                    ...
                    'dinf' => $externoData['es_dinf'], // Tu BD fantasma lo llama 'es_dinf'
                ]
            );
        }
    ...
    private function sincronizarNotas()
    {
        ...
        $notasExternas = DB::connection('pgsql_ucsc')->table('public.notas_finales_externas')->get();
        ...
        foreach ($notasExternas as $externo) {
            ...
            // (Opcional: quitamos ->where('nota', '!=', $externoData['nota_final']) si quieres registrar la fecha siempre)
